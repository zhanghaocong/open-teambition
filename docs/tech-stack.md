# open-teambition 技术选型与架构决策

> 本文档记录 open-teambition 的技术选型与关键架构决策，供评审。
...
**收益**：业务逻辑只写一次；新增能力时四个入口同时具备，无需分别实现。这是整套设计的地基。
...
### 4.6 CLI（`apps/cli`）与 MCP（`apps/mcp`）
- 均为**薄适配器**，不含业务逻辑，仅做协议转换。
- 与 Web 一起在 MVP 交付。

## 5. 关键架构决策（ADR 摘要）

### ADR-1：能力注册表为核心，四适配器薄封装
见第 2 节。**避免的返工**：业务逻辑只写一次，防止为 CLI/MCP/Agent 重复实现或 hack。

### ADR-2：事件流 / live query 用 Postgres `LISTEN/NOTIFY`，MVP 不引入 Redis
- 所有状态变更发领域事件，同时驱动：前端实时更新、审计日志、Agent 响应式触发。
- MVP 用 Postgres `LISTEN/NOTIFY` 承载事件总线与 live query 触发，**不引入 Redis**。
- Redis 作为后续横向扩展项（多节点 pub/sub + 缓存）预留抽象层。
- **live query 形态**：在 core 注册命名查询，相关 `NOTIFY` 到达时重算，结果集变化即触发。

### ADR-3：多主体身份 + 审计 + 幂等 + 结构化错误（Agent 前置约束）
...
- **避免的返工**：鉴权/审计/幂等若后加，等于重写所有入口。

### ADR-4：只用标准 SQL 查询，不引入 pgvector / 向量检索
- 项目/任务检索与 live query 一律用标准 SQL 查询（Drizzle）实现。
- **不引入** `pgvector` 依赖或向量数据结构。
- **不为向量检索预留任何接口或抽象**——是否采用向量检索不作预设，需要时再按当时需求评估设计，避免过度设计。
...
3. **首期功能范围**：拟定为最小垂直切片——项目 / 看板任务（建、移动、查）+ 一个响应式 Agent 触发（任务逾期）+ 一个对话式 MCP 工具入口。是否加/减？
...
| 方案 | 结论 | 理由 |
|------|------|------|
| Cloudflare Workers + Durable Objects（方案 B） | **未采用** | 已定 Node 自托管；更符合开源可自托管预期 |
| Redis（事件总线/缓存） | **延后** | MVP 用 Postgres `LISTEN/NOTIFY` 即可；多节点扩展时再引入 |
| pgvector / 语义检索 | **不采用** | 一律用标准查询；不预设、不预留接口 |
| Yjs（CRDT 文档协同） | **延后** | 首期聚焦项目/看板；富文本协同后续再评估 |
